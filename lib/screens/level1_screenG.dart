...
class Level1ScreenG extends StatefulWidget {
  ...
  @override
  State<Level1ScreenG> createState() => _Level1ScreenGState();
}

class _Level1ScreenGState extends State<Level1ScreenG> {
  ...
  Future<void> requestPermissions() async {
    ...
    if (mic.isGranted && storage.isGranted) {
      ...
      // Si lo quieres, aquí puedes mostrar SnackBar
  ...
  Future<void> startRecording() async {
    ...
    final audioFilePath = '$tempDir/audio_temp_${widget.activityId}.wav';
  ...
  Future<void> fetchPhonologicalAwareness(int id) async {
    try {
      final response = await http.get(Uri.parse(Api.phonologicalById(id)));
  ...
  Future<void> fetchFeedback(int activityId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // tu backend devuelve lista (según tu código viejo)
        if (jsonResponse is List && jsonResponse.isNotEmpty) {
          final firstActivity = jsonResponse[0];
          setState(() {
            feedbackImageG = firstActivity['image'] ?? '';
            feedbackMessageG = firstActivity['feedback'] ?? '';
          });
        }
      } else {
        // print('Feedback failed: ${response.statusCode}');
      }
    } catch (e) {
      // print(e.toString());
    }
  ...
  Future<void> saveProgress({required int score}) async {
    ...
    // tu backend: POST /progress/phonological/{activityId}?uid=...&score=...
    final url =
        "${Api.savePhonologicalProgress(widget.activityId)}?uid=$uid&score=$score";
  ...
  bool fuzzyMatch(String spokenRaw, String correctRaw) {
    ...
    // Si Whisper devuelve varias palabras, probamos cada token y nos quedamos con el mejor
