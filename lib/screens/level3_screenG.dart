...
class Level3ScreenG extends StatefulWidget {
  final int activityId; // 21..30
  const Level3ScreenG({super.key, required this.activityId});

  @override
  State<Level3ScreenG> createState() => _Level3ScreenGState();
}

class _Level3ScreenGState extends State<Level3ScreenG> {
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  final FirebaseAuthServices _auth = FirebaseAuthServices();

  static const int startId = 1;
  static const int endId = 30;

  UserModel? _user;
  ReadingComprehensionModel? readingModel;
  ...
  Future<void> startRecording() async {
    ...
    final audioFilePath = '$tempDir/audio_level3_${widget.activityId}.wav';
  ...
  Future<void> stopRecording() async {
    try {
      ...
      final correctRaw = readingModel?.correct_answer ?? '';
      print('✅ RESPUESTA CORRECTA: "$correctRaw"');

      // Evaluación con IA (embedding semántico + léxico + fonético)
      final aiResult = await evaluateWithAI(transcribed, correctRaw, 'reading_comp');
  ...
  Future<void> fetchReading(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          readingModel = ReadingComprehensionModel.fromMap(jsonData);
        });
      }
    } catch (_) {}
  }
  ...
  Future<void> fetchFeedback(int activityId) async {
    try {
      // mismo patrón que tu código viejo, pero usando Api.baseUrl
      final url = "${Api.baseUrl}/reading/$activityId";
  ...
  Future<void> updateUserResponse(String userResponse,
    ...
    final update = ReadingComprehensionModel(
      ...
      main_image: readingModel?.main_image ?? '',
      question: readingModel?.question ?? '',
      statement: readingModel?.statement ?? '',
      user_response: userResponse,
      correct_answer: readingModel?.correct_answer ?? '',
      correct: correct,
      level_id: readingModel?.level_id ?? 0,
      answer_one: readingModel?.answer_one ?? '',
      answer_two: readingModel?.answer_two ?? '',
      answer_three: readingModel?.answer_three ?? '',
    );

    try {
      final response = await http.put(
        Uri.parse(Api.readingById(readingModel?.id ?? widget.activityId)),
        headers: {'Content-Type': 'application/json; charset=UTF-8'},
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: IconButton(
                      ...
                      onPressed: () async {
                        await speak(
                          'Enunciado. ${readingModel?.statement ?? ''}. '
                          'Pregunta. ${readingModel?.question ?? ''}.',
                        );
                      },
                    ),
                  ),
              ...
              ButtonActivities(
                ...
                onPressed: () async {
                  ...
                  if (userResponse.isNotEmpty) {
                    final correct = readingModel?.correct_answer ?? '';
