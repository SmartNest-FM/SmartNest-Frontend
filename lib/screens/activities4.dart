...
import 'package:smartnest/screens/level4_screenG.dart';
...
class _Activities4ScreenState extends State<Activities4Screen> {
  ...
  final List<CombinationReadingImagesModel?> combo = List.filled(endId + 1, null);
  ...
  void initState() {
    ...
    speak(
      "Bienvenido al nivel 4, aquí encontrarás 10 actividades para mejorar la comprensión lectora combinando lectura e imágenes.",
    );
  }
  ...
  Future<void> loadLevelPercentage() async {
    ...
    final url = "${Api.baseUrl}/progress/percentage/4?uid=$uid";
  ...
  Future<void> loadAllCombination() async {
    for (int id = startId; id <= endId; id++) {
      await fetchCombination(id);
    }
  }
  ...
  Future<void> fetchCombination(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          combo[id] = CombinationReadingImagesModel.fromMap(jsonData);
        });
      }
    } catch (e) {
      print("fetchCombination($id) error: $e");
    }
  }

  Future<bool> fetchActivityStatus(int id) async {
    final uid = _auth.currentUser!.uid;
    final url = "${Api.baseUrl}/progress/status/COMBINATION/$id?uid=$uid";
  ...
  Widget activityItem(int realId) {
    final shown = realId - startId + 1; // 31->1 ... 40->10
      ...
      children: [
        ButtonActivities(
          ...
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => Level4ScreenG(activityId: realId)),
            );
          },
        ),
        ClipOval(
